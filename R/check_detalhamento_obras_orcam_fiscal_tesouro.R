...
#'
#' @export
check_detalhamento_obras_orcam_fiscal_tesouro <- function(base_qdd_fiscal,
  ...
  x <- base_qdd_fiscal |> 
    aggregate("vlr_loa_desp$",
              by = key,
              filter = elemento_cod == 51 & fonte_cod %in% c(10, 15)
              )

  y <- base_detalhamento_obras |> 
    aggregate("vlr_tesouro_ano0",
              by = key,
  ...
  default_message = paste0(
                    ...
                    "valores do Tesouro diferentes entre as bases, R$ {vlr_loa_desp} na qdd-fiscal e R$ ",
                    "{vlr_tesouro_ano0} na detalhamento de obras."
                    )
