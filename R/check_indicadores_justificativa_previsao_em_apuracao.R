...
check_indicadores_justificativa_previsao_em_apuracao <- function(indicadores_planejamento,
  ...
  default_message = "O programa {programa_cod} contém indicador ({paste(unlist(strsplit(indicador, ' '))[1:3], collapse = ' ')}...) que, apesar de estar sem previsão para o ano {ifelse(is.na(previsao_para_ano0), 0, ifelse(is.na(previsao_para_ano1), 1, ifelse(is.na(previsao_para_ano2), 2, 3)))}, apresenta justificativa de previsão inválida, com menos de 50 caracteres {ifelse(is.na(justificativa_status_apuracao_previsoes), '(0 caract.)', paste0('(', stringr::str_length(justificativa_status_apuracao_previsoes), ' caract. - ', paste(unlist(strsplit(justificativa_status_apuracao_previsoes, ' '))[1:3], collapse = ' '), '...)'))}."
